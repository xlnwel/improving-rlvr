...
SYSTEM_PROMPT = "I am going to give you a series of demonstrations of math Problems and Solutions. When you respond, respond only with the Solution of the final Problem. "
...
def parse_args():
    ...
    parser.add_argument("--demo_type", "-dt", type=str, default='sample')
...
def eval_model(args, model_path, dataset, batch_size=100):
    ...
    output_dir = f'icl/results/n={n}-k={k}/{model_name}-instruction_type={args.instruction_type}-{n_demos}demos'
    ...
    similarity_path = 'icl/results/all-mpnet-base-v2_similarity.npy'
    ...
    for i in tqdm(range(0, len(dataset), batch_size)):
        ...
        for j, (p, s, a) in enumerate(zip(problems, solutions, answers)):
            ...
            elif args.demo_type == 'similar':
                demo_indices = list(range(n_dataset_demos))
            else:
        ...
        if args.instruction_type == 'system':
            ...
            prompts = ['\n'.join(SYSTEM_PROMPT, prompt) for problem in problems]
        else:
...
def main():
    ...
    aime24_dataset = pd.read_csv('icl/results/icl_aime2024.csv')
